...
router.post("/signup", async (req, res) => {
  try {
    ...
    let orgId = null;
    if (role === "user") {
      if (!joinCode) return res.status(400).json({ message: "Join code is required for user role" });
      const department = await Organization.findOne({ joinCode, type: "department" });
      if (!department) return res.status(400).json({ message: "Invalid join code" });
      orgId = department._id;
    } else {
      if (joinCode) return res.status(400).json({ message: "Join code not allowed for admin roles" });
    }
...
// Get current user + organization hierarchy
