...
// ------------------------------------
const Navbar = () => {
  // ✅ Use isAuthenticated (not token) — stays in sync with user state
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-white/50 backdrop-blur-xl border-b border-white/30 shadow-[0_4px_12px_rgba(0,0,0,0.08)]">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        {/* DESKTOP NAV */}
        <div className="hidden md:flex items-center gap-2 p-1 bg-white/40 backdrop-blur-xl rounded-full border border-white/30 shadow-sm">

          {/* Always show common links */}
          {commonLinks.map((l) => (
            <NavLink
              key={l.to}
              to={l.to}
          ...
          {/* ✅ Logged in — show admin dropdown, user info, logout */}
          {isAuthenticated && (
            <div className="flex items-center gap-2 ml-2">
              {isAdmin && (
                <DropdownMenu trigger={<Settings className="h-5 w-5 text-[#0A1A4F]" />}>
                  ...
                  {adminLinks.map((a) => (
                    <DropdownItem key={a.to} to={a.to} label={a.label} Icon={a.icon} />
                  ))}
                </DropdownMenu>
              )}
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-red-600 border border-red-300 rounded-full hover:bg-red-50 transition"
              >
        ...
        {/* MOBILE TOGGLE */}
        <button className="md:hidden p-2" onClick={() => setMobileMenu((p) => !p)}>
      ...
      <AnimatePresence>
        {mobileMenu && (
          <motion.div
            ...
            className="md:hidden origin-top bg-white/50 backdrop-blur-xl border-t border-white/30"
          >
            <div className="py-3 flex flex-col gap-1">
              {commonLinks.map((l) => (
                <NavLink
                  ...
                  className="flex items-center gap-3 px-4 py-2 hover:bg-white/40 transition rounded-md"
                  style={{ color: "#0A1A4F", fontWeight: 600 }}
                >
              ...
              {/* ✅ Mobile logout also goes to /auth */}
              {isAuthenticated ? (
                <button
                  onClick={handleLogout}
                  className="flex items-center justify-center gap-2 w-[90%] mx-auto mt-3 px-4 py-2 rounded-full border border-red-300 text-red-600 hover:bg-red-50 transition"
                >
                ...
                <NavLink
                  to="/auth"
                  onClick={() => setMobileMenu(false)}
                  className="px-4 py-2 text-indigo-600 text-center border border-indigo-300 rounded-full mx-4 hover:bg-indigo-50 transition"
                >
                  Login / Signup
                </NavLink>
              )}
